...
function toRouterStateMatches(matches) {
  return matches.map((m) => ({ id: m.route.id, pathname: m.pathname }));
}
function useRouterState() {
  let state = useDataRouterState("unstable_useRouterState" /* UseRouterState */);
  return React3.useMemo(() => {
    let leaf = state.matches[state.matches.length - 1];
    let active = {
      location: state.location,
      searchParams: new URLSearchParams(state.location.search),
      params: _nullishCoalesce(_optionalChain([leaf, 'optionalAccess', _103 => _103.params]), () => ( {})),
    ...
    if (nav.state !== "idle") {
      ...
      pending = {
        ...
        params: _nullishCoalesce(_optionalChain([pendingLeaf, 'optionalAccess', _104 => _104.params]), () => ( {})),
...
function getKeyedLinksForMatches(matches, routeModules, manifest) {
  let descriptors = matches.map((match) => {
    ...
    return [
      ...
      _optionalChain([module, 'optionalAccess', _105 => _105.links, 'optionalCall', _106 => _106()]) || []
    ];
  }).flat(2);
...
function getNewMatchesForLinks(page, nextMatches, currentMatches, manifest, location, mode) {
  ...
  let matchPathChanged = (match, index) => {
    return (
      ...
      // e.g. /files/images/avatar.jpg -> files/finances.xls
      _optionalChain([currentMatches, 'access', _107 => _107[index], 'access', _108 => _108.route, 'access', _109 => _109.path, 'optionalAccess', _110 => _110.endsWith, 'call', _111 => _111("*")]) && currentMatches[index].params["*"] !== match.params["*"]
    );
  };
  if (mode === "assets") {
    ...
    return nextMatches.filter((match, index) => {
      ...
      if (match.route.shouldRevalidate) {
        let routeChoice = match.route.shouldRevalidate({
          ...
          currentParams: _optionalChain([currentMatches, 'access', _112 => _112[0], 'optionalAccess', _113 => _113.params]) || {},
...
function createClientRoutes(manifest, routeModulesCache, initialState, ssr, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), needsRevalidation) {
  return (routesByParentId[parentId] || []).map((route) => {
    ...
    if (routeModule) {
      ...
      let initialData = hasInitialData ? _optionalChain([initialState, 'optionalAccess', _114 => _114.loaderData, 'optionalAccess', _115 => _115[route.id]]) : void 0;
      let hasInitialError = initialState && initialState.errors && route.id in initialState.errors;
      let initialError = hasInitialError ? _optionalChain([initialState, 'optionalAccess', _116 => _116.errors, 'optionalAccess', _117 => _117[route.id]]) : void 0;
      let isHydrationRequest = needsRevalidation == null && (_optionalChain([routeModule, 'access', _118 => _118.clientLoader, 'optionalAccess', _119 => _119.hydrate]) === true || !route.hasLoader);
...
function useFogOFWarDiscovery(router, manifest, routeModules, ssr, routeDiscovery, isSpaMode) {
  React6.useEffect(() => {
    ...
    _optionalChain([window, 'access', _120 => _120.navigator, 'optionalAccess', _121 => _121.connection, 'optionalAccess', _122 => _122.saveData]) === true) {
      return;
    }
...
async function fetchAndApplyManifestPatches(paths, errorReloadPath, manifest, routeModules, ssr, isSpaMode, basename, manifestPath, patchRoutes, signal) {
  ...
  try {
    ...
    if (_optionalChain([signal, 'optionalAccess', _123 => _123.aborted])) return;
    throw e;
  }
...
function PrefetchPageLinksImpl({
  ...
  let dataHrefs = React7.useMemo(() => {
    ...
    nextMatches.forEach((m) => {
      ...
      if (!newMatchesForData.some((m2) => m2.route.id === m.route.id) && m.route.id in loaderData && _optionalChain([routeModules, 'access', _124 => _124[m.route.id], 'optionalAccess', _125 => _125.shouldRevalidate])) {
        foundOptOutRoute = true;
...
function Meta() {
  ...
  for (let i = 0; i < _matches.length; i++) {
    ...
    matches[i] = match;
    if (_optionalChain([routeModule, 'optionalAccess', _126 => _126.meta])) {
...
function Scripts(scriptProps) {
  ...
  let initialScripts = React7.useMemo(() => {
    ...
    let routeModulesScript = !isStatic ? " " : `${_optionalChain([manifest, 'access', _127 => _127.hmr, 'optionalAccess', _128 => _128.runtime]) ? `import ${JSON.stringify(manifest.hmr.runtime)};` : ""}${!enableFogOfWar ? `import ${JSON.stringify(manifest.url)}` : ""};
...
function BoundaryShell({
  ...
  if (_optionalChain([routeModules, 'access', _129 => _129.root, 'optionalAccess', _130 => _130.Layout]) && !isOutsideRemixApp) {
    return children;
  }
...
function createMemoryRouter(routes, opts) {
  return createRouter({
    basename: _optionalChain([opts, 'optionalAccess', _131 => _131.basename]),
    getContext: _optionalChain([opts, 'optionalAccess', _132 => _132.getContext]),
    future: _optionalChain([opts, 'optionalAccess', _133 => _133.future]),
    history: createMemoryHistory({
      initialEntries: _optionalChain([opts, 'optionalAccess', _134 => _134.initialEntries]),
      initialIndex: _optionalChain([opts, 'optionalAccess', _135 => _135.initialIndex])
    }),
    hydrationData: _optionalChain([opts, 'optionalAccess', _136 => _136.hydrationData]),
    routes,
    hydrationRouteProperties,
    mapRouteProperties,
    dataStrategy: _optionalChain([opts, 'optionalAccess', _137 => _137.dataStrategy]),
    patchRoutesOnNavigation: _optionalChain([opts, 'optionalAccess', _138 => _138.patchRoutesOnNavigation]),
    instrumentations: _optionalChain([opts, 'optionalAccess', _139 => _139.instrumentations])
  }).initialize();
}
var Deferred2 = class {
  constructor() {
...
function RouterProvider({
  ...
  let setState = React9.useCallback(
    (newState, { deletedFetchers, newErrors, flushSync, viewTransitionOpts }) => {
      if (newErrors && onError) {
        Object.values(newErrors).forEach(
          (error) => onError(error, {
            ...
            params: _nullishCoalesce(_optionalChain([newState, 'access', _140 => _140.matches, 'access', _141 => _141[0], 'optionalAccess', _142 => _142.params]), () => ( {})),
      ...
      if (transition) {
        _optionalChain([renderDfd, 'optionalAccess', _145 => _145.resolve, 'call', _146 => _146()]);
        transition.skipTransition();
        setInterruption({
          state: newState,
          currentLocation: viewTransitionOpts.currentLocation,
  ...
  let navigator = React9.useMemo(() => {
    return {
      ...
      replace: (to, state2, opts) => router.navigate(to, {
        replace: true,
        state: state2,
        preventScrollReset: _optionalChain([opts, 'optionalAccess', _148 => _148.preventScrollReset])
      })
    };
  }, [router]);
    ...
    (error, errorInfo) => {
      if (dataRouterContext && dataRouterContext.onError && dataRouterStateContext) {
        dataRouterContext.onError(error, {
          ...
          params: _optionalChain([dataRouterStateContext, 'access', _149 => _149.matches, 'access', _150 => _150[0], 'optionalAccess', _151 => _151.params]) || {},
  ...
  render() {
    ...
    if (!(resolve instanceof Promise)) {
      ...
      promise = resolve.then(
        ...
        (error) => {
          _optionalChain([this, 'access', _152 => _152.props, 'access', _153 => _153.onError, 'optionalCall', _154 => _154(error)]);
